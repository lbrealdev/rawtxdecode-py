...
def main():
    ...
    tx_output = create_tx_output(decode_tx, public_key)

    # json output `default`
    # tx_json_output = json.dumps(tx_output, indent=2)
    # print(tx_json_output)

    # json output `stringify`
    tx_output_formatted = stringify_json(tx_output)
    tx_json_output = json.dumps(tx_output_formatted, indent=2)
    print(tx_json_output)
